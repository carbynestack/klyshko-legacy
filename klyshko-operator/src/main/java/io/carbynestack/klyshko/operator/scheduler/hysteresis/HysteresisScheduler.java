...
import io.carbynestack.klyshko.operator.scheduler.SchedulerSpec;
 ...
 *
 * <p>The scheduler operates in a master/follower model using a shared etcd server to coordinate
 * actions.
 *
 * <p>The master discovers the castor service (by name, currently hard-coded as "cs-castor") and
 * periodically fetches telemetry data. Tuple generation is started when the number of available
 * tuples drops below a given threshold and stopped when it exceeds an upper threshold (see {@link
 * #controlLoop()}).
 *
 * <p>The scheduler periodically checks (see {@link #schedule()}), whether tuples of certain type
 * should be generated and instructs all VCPs in a VC to create a respective job by instantiating an
 * etcd-backed job roster used to track the states of all correlated jobs across the VCPs in the VC.
 * VCP-local job management is delegated to an instance of {@link VcpJobManager}. Job rosters on
 * which the job on all VCPs have been completed are removed by removing the respective K/V entry in
 * etcd.
 *
 * <p>The number of (per type) tuple generation jobs is limited by configurable upper limit.
 */
public class HysteresisScheduler implements Closeable, io.etcd.jetcd.Watch.Listener {
  ...
  public HysteresisScheduler(
      KubernetesClient k8sClient, Scheduler schedulerResource, ObjectMapper objectMapper) {
    ...
    String etcdEndpoint =
        String.format("http://%s:%d", schedulerResource.getSpec().etcdEndpoint(), 2379);
    Log.infof("Using etcd at endpoint %s", etcdEndpoint);
    etcdClient =
        Client.builder()
            .endpoints(etcdEndpoint)
            .namespace(ByteSequence.from(ETCD_PREFIX, StandardCharsets.UTF_8))
            .build();
      ...
      jobWatcher =
          Optional.of(
              etcdClient
                  .getWatchClient()
                  .watch(
                      new JobRosterDirectoryKey().toEtcdKey(),
                      WatchOption.newBuilder().isPrefix(true).withNoDelete(true).build(),
                      this));
      ses.scheduleWithFixedDelay(
          this::controlLoop, 0, 5, TimeUnit.SECONDS); // TODO: Make configurable
      ses.scheduleWithFixedDelay(
          this::schedule, 0, 10, TimeUnit.SECONDS); // TODO: Make configurable
    } else {
      jobWatcher = Optional.empty();
    }
    Log.infof(
        "Scheduler instance %s created, acting as %s",
        schedulerResource.getMetadata().getName(), isMaster ? "master" : "follower");
  }
  ...
  private State computeState(State currentState, SchedulerSpec spec, TupleMetric m) {
    if (State.IDLE.equals(currentState) && m.getAvailable() < spec.lowerThreshold()) {
      Log.infof(
          "Available tuples (%d) for type %s below lower threshold (%d) - start generating",
          m.getAvailable(), m.getType(), spec.lowerThreshold());
      return State.GENERATING;
    } else if (State.GENERATING.equals(currentState) && m.getAvailable() > spec.upperThreshold()) {
      Log.infof(
          "Available tuples (%d) for type %s above upper threshold (%d) - stop generating",
  ...
  public void controlLoop() {
    ...
    try {
      ...
      var castorService =
          k8sClient.services().withName("cs-castor"); // TODO Make service name configurable
      ...
      String castorEndpoint =
          String.format(
              ...
              castorSpec
                  .getPorts()
                  .get(0)
                  .getPort()); // TODO Handle case when port is not given (possible?)
      Log.debugf("Castor service discovered at endpoint %s", castorEndpoint);
      CastorIntraVcpClient castorIntraVcpClient =
          DefaultCastorIntraVcpClient.builder(castorEndpoint)
      ...
      telemetryData
          ...
          .forEach(
              m -> {
                ...
                states.put(m.getType(), computeState(currentState, spec, m));
              });
    } catch (Exception e) {
  ...
  private CompletableFuture<Long> getNumberOfActiveJobs(KV kv) {
    return kv.get(
            JobRosterDirectoryKey.INSTANCE.toEtcdKey(),
            GetOption.newBuilder().isPrefix(true).build())
        .thenApply(
            r ->
                r.getKvs().stream()
                    .map(KeyValue::getKey)
                    .filter(k -> Key.fromEtcdKeyOptional(k, JobRosterKey.class).isPresent())
                    .count());
  }
  ...
  void schedule() {
    try {
      ...
      getNumberOfActiveJobs(kv)
          .thenAccept(
              c -> {
                if (c >= spec.parallelism()) {
                  Log.infof(
                      "At maximum allowed parallelism (%d of %d jobs) - no jobs to be scheduled",
                      c, spec.parallelism());
                  return;
                }
                var generating =
                    states.entrySet().stream()
                        .filter(e -> State.GENERATING.equals(e.getValue()))
                      ...
                      .ifPresent(
                          throwingConsumer(
                              t -> {
                                ...
                                var rosterKeyEntryOps =
                                    IntStream.range(0, NUMBER_OF_PARTIES)
                                        .mapToObj(
                                            p ->
                                                Op.put(
                                                    rosterKey.getJobRosterEntryKey(p).toEtcdKey(),
                                                    ByteSequence.from(
                                                        JobState.CREATED.toString(),
                                ...
                                kv.txn()
                                    ...
                                    .Else(
                                        Op.put(
                                            rosterKey.toEtcdKey(),
                                            ByteSequence.from(
                                                objectMapper.writeValueAsBytes(jobData)),
                                            PutOption.DEFAULT))
                                    ...
                                    .thenAccept(
                                        r -> {
                                          ...
                                          Log.logf(
                                              level,
                                              "Roster creation for job with tuple type %s and ID %s"
                                                  + " %s",
                                              t,
                                              rosterKey.jobId(),
                                              r.isSucceeded() ? "successful" : "failed");
                                        });
                              }));
                }
              })
  ...
  public void onNext(WatchResponse watchResponse) {
    if (Log.isDebugEnabled()) {
      Log.debugf(
          "Etcd watch triggered for keys: %s",
          watchResponse.getEvents().stream()
              .map(e -> e.getKeyValue().getKey())
    ...
    for (var event : watchResponse.getEvents()) {
      ...
      Key.fromEtcdKeyOptional(key, JobRosterEntryKey.class)
          .ifPresentOrElse(
              k -> {
                UUID jobId = k.jobId();
                ...
                kvClient
                    .get(rosterPrefixKey, GetOption.newBuilder().isPrefix(true).build())
                    .thenApply(
                        r -> {
                          ...
                          if (Log.isDebugEnabled()) {
                            Log.debugf(
                                "New VC job state '%s' computed from VCP job states '%s'",
                                newVcJobState, vcpJobStates);
                          }
                                            ...
                                            throwingFunction(
                                                v ->
                                                    objectMapper.readValue(
                                                        v.getValue().getBytes(), JobData.class)))),
                                    ...
                                    if (jd.jobState() != newVcJobState) {
                                      ...
                                      kvClient
                                          .put(
                                              rosterKey,
                                              ByteSequence.from(
                                                  objectMapper.writeValueAsBytes(njd)))
                                          .join();
                                      Log.infof(
                                          "Roster for job with ID %s updated to new VC job state"
                                              + " %s",
                                          jobId, newVcJobState);
                                    }
                                  }));
                    ...
                    .thenCompose(
                        jobState -> {
                          ...
                          if (EnumSet.of(JobState.SUCCEEDED, JobState.FAILED).contains(jobState)) {
                            Log.infof(
                                "Jobs on all parties completed for ID %s with state %s",
                                jobId, jobState);
                            return kvClient
                                .delete(rosterKey, DeleteOption.newBuilder().isPrefix(true).build())
                                .thenRun(() -> Log.infof("Job roster for ID %s deleted", jobId));
                          } else {
  ...
  public void onError(Throwable throwable) {
    Log.errorf(
        throwable, "Error for watch on scheduler %s", schedulerResource.getMetadata().getName());
  }

  @Override
  public void onCompleted() {
    Log.debugf(
        "Watch result processing completed on scheduler %s",
        schedulerResource.getMetadata().getName());
  }
}

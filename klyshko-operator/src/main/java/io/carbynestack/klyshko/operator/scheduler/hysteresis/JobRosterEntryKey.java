...
public record JobRosterEntryKey(UUID jobId, int party) implements Key {
    ...
    private static final Pattern PATTERN = Pattern.compile("^/jobs/([0-9a-f]{8}\\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4" +
            "}-\\b[0-9a-f]{12})/(\\d+)$");
    ...
    static Tuple2<UUID, Integer> parse(ByteSequence etcdKey) {
        ...
        if (!m.matches())
            throw new IllegalArgumentException(String.format("'%s' is not a job roster entry key", etcdKeyStr));

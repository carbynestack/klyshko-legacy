 ...
 *
 * <p>Watches etcd for creation of job roster entries and creates the corresponding local tuple
 * generation job as a K8s job. When a job roster is deleted from etcd the associated local K8s job
 * gets deleted. In case tuples have been successfully created by all VCPs, the respective tuple
 * chunk is activated.
 *
 * <p>The etcd job roster state is updated based on the VCP-local K8s job state.
 */
class VcpJobManager implements io.etcd.jetcd.Watch.Listener, Watcher<Pod>, Closeable {
  ...
  public void onNext(WatchResponse watchResponse) {
    ...
    for (var event : watchResponse.getEvents()) {
      ...
      var key = kv.getKey();
      Key.fromEtcdKeyOptional(key, JobRosterKey.class)
          .ifPresentOrElse(
              k -> {
                ...
                try {
                  switch (event.getEventType()) {
                    case PUT -> {
                      ...
                      JobData jobData =
                          objectMapper.readValue(kv.getValue().getBytes(), JobData.class);
                    ...
                    case DELETE -> {
                      ...
                      JobData jobData =
                          objectMapper.readValue(
                              event.getPrevKV().getValue().getBytes(), JobData.class);
  ...
  public void onError(Throwable throwable) {
    Log.errorf(
        throwable,
        "Error for watch on job manager for scheduler %s",
  ...
  public void onCompleted() {
    Log.debugf(
        "Watch result processing completed on job manager for scheduler %s",
        scheduler.getMetadata().getName());
  }

  void createVcpLocalJob(JobRosterKey key, JobParameters params) {
    Log.debugf(
        "Creating VCP-local job for VC job with ID '%s' using parameters: %s", key.jobId(), params);
    Try.of(() -> IOUtils.resourceToString("/kii-launch-script.sh", StandardCharsets.UTF_8))
        .onSuccess(
            kiiLaunchScript -> {
              var job = k8sClient.pods().create(buildVcpLocalJobPod(key, params, kiiLaunchScript));
              Log.infof(
                  "VCP-local job with name %s created for VC job %s",
                  job.getMetadata().getName(), key.jobId());
            })
        .onFailure(
            e ->
                Log.errorf(e, "Failed to launch VCP-local job for VC job with ID %s", key.jobId()));
  }
  ...
  private Pod buildVcpLocalJobPod(JobRosterKey key, JobParameters params, String kiiLaunchScript) {
    ...
    return new PodBuilder()
        ...
        .withLabels(
            Map.of(
                // TODO Add other recommended labels (see
        ...
        .withEnv(
            ...
            new EnvVarBuilder()
                ...
                .build(),
            new EnvVarBuilder().withName("KII_TUPLE_FILE").withValue("/kii/tuples").build())
        .withVolumeMounts(
            new VolumeMountBuilder().withName("kii").withMountPath(kiiSharedFolderPath).build(),
        ...
        .withEnv(
            ...
            new EnvVarBuilder()
                ...
                .withValue(params.tupleType().toString())
                .build(),
            new EnvVarBuilder().withName("KII_SHARED_FOLDER").withValue("/kii").build(),
            new EnvVarBuilder().withName("KII_TUPLE_FILE").withValue("/kii/tuples").build())
        .withVolumeMounts(new VolumeMountBuilder().withName("kii").withMountPath("/kii").build())
        .endContainer()
        .withVolumes(
            new VolumeBuilder().withName("kii").withNewEmptyDir().and().build(),
            new VolumeBuilder()
                .withName("params")
                .withConfigMap(
  ...
  void deleteJob(JobRosterKey key) {
    boolean deleted =
        k8sClient
            .pods()
            .withLabel("klyshko.carbynestack.io/jobId", key.jobId().toString())
            .delete();
  ...
  public void eventReceived(Action action, Pod pod) {
    ...
    Log.debugf(
        "processing Klyshko-managed VCP job %s with status %s and key %s",
        pod.getMetadata().getName(), pod.getStatus(), key);
    // TODO Use configured player number
    JobRosterEntryKey jreKey =
        new JobRosterEntryKey(key.jobId(), scheduler.getSpec().master() ? 0 : 1);
    toVcpJobState(pod.getStatus())
        .ifPresent(
            s -> {
  ...
  void activateTuples(UUID jobId) {
    getCastorUploadClient()
        .ifPresentOrElse(
            ...
            () ->
                Log.warnf(
                    "Skipping tuple chunk activation for job with id '%s' due to unavailable Castor"
                        + " service",
                    jobId));
  }
  ...
  Optional<CastorUploadClient> getCastorUploadClient() {
    ...
    return Optional.of(
        DefaultCastorUploadClient.builder(castorEndpoint)
